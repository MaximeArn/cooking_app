...
class _TrimEditorState extends State<TrimEditor> with TickerProviderStateMixin {
  ...
  /// Quick access to CachedVideoPlayerController, only not null after [TrimmerEvent.initialized]
  /// has been emitted.
get CachedVideoPlayerController =>
  ...
  void initState() {
    ...
    widget.trimmer.eventStream.listen((event) {
      if (event == TrimmerEvent.initialized) {
        ...
        CachedVideoPlayerController.seekTo(const Duration(milliseconds: 0));
        setState(() {
          Duration totalDuration = CachedVideoPlayerController.value.duration;
  ...
  Future<void> _initializeVideoController() async {
    if (_videoFile != null) {
      CachedVideoPlayerController.addListener(() {
        final bool isPlaying = CachedVideoPlayerController.value.isPlaying;

        if (isPlaying) {
          widget.onChangePlaybackState!(true);
          setState(() {
            _currentPosition =
                CachedVideoPlayerController.value.position.inMilliseconds;
          ...
          if (CachedVideoPlayerController.value.isInitialized) {
      ...
      _videoDuration = CachedVideoPlayerController.value.duration.inMilliseconds;
  ...
  void dispose() {
    ...
    widget.onChangePlaybackState!(false);
    if (_videoFile != null) {
      CachedVideoPlayerController.setVolume(0.0);
                  ...
                  child: Padding(
                    ...
                    child: Row(
                      ...
                      children: <Widget>[
                        ...
                        CachedVideoPlayerController.value.isPlaying
                            ? Text(
